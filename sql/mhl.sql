-- 创建满汉楼的数据库
CREATE DATABASE mhl;
-- 创建表 employee 表 （主键id,empId,name,pwd,job等 如果需要可以自己加字段等)
-- 用户表
...
# 添加测试数据
  ...
  -- 创建dinngTable 表(id,state,order,orderName,orderTel ....)
...
CREATE TABLE bill (
	...
	billed VARCHAR(50) NOT NULL DEFAULT '',#账单号可以按照自己规则生成UUID
	menuId INT NOT NULL DEFAULT 0,#菜品的编号，也可以使用外建
