-- 7. 根据：emp员工表，dept部门表，工资 = 薪金sal + 佣金comm写出正确sql
-- 1. 列出至少有一个员工的所有部门
/*
	先查出各个部门有多少人
	使用 having 子句过滤
...
-- 条件1.worker.hiredate > leader.hiredate
...
-- 就是emp和dept联合查询，连接条件就是emp.deptno = dept.deptno
-- 13. 列出所有部门的详细信息和部门人数
-- 1. 先得到各个部门的人数，把下面的结果看做临时表,和dept表联合查询
SELECT COUNT(*) AS c,deptno
	FROM emp
	GROUP BY deptno
-- 2. 
