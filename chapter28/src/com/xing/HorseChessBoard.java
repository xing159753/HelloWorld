...
public class HorseChessBoard {
    ...
    //写一个方法，对ps的各个位置，可以走的下一个位置的次数进行排序，把可能走的下一一个位置从小到大排序
    ...
    public static void traversalChessBoard(int[][] chessBoard, int row, int col, int step) {
        ...
        while (!ps.isEmpty()) {
            //取出一个位置点
            Point p = ps.remove(0);
            //判断改为置是否走过，如果没有走过，我们就递归遍历
            if (!visited[p.y * X + p.x]) {
                //递归遍历
                traversalChessBoard(chessBoard, p.y, p.x, step + 1);
            }
        }
        //当退出while,看看是否遍历成功，如果没有成功，就重置相应的值，然后进行会输
    ...
    public static ArrayList<Point> next(Point curPoint) {
        ...
        Point p1 = new Point();
        //判断 curPoint 是否可以走如下位置，如果可以走，就将该点(Point) 放入到ps

        //判断是否可以走5位置
        if ((p1.x = curPoint.x - 2) >= 0 && (p1.y = curPoint.y - 1) >= 0) {
            ps.add(new Point(p1));//这里一定要 new Point
        }
        //判断是否可以走6位置
