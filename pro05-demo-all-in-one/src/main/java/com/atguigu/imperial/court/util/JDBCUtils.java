...
public class JDBCUtils {
    ...
    static {
        ...
        try {
            ...
            // new FileInputStream("D:\\code\\HelloWorld\\pro05-demo-all-in-one\\src\\main\\resources\\jdbc.properties");
            // 为了保证程序的可移植性，需要基于一个确定的基准来读取这个文件
            // 确定的基准：类路径的根路径。resources 目录下的内容经过构建操作中的打包操作后会确定放在 WEB-INF/classes 目录下。
            // WEB-INF/classes 目录存放编译好的*.class 字节码文件，所以这个目录我们就称为类路径。
            // 类路径无论在本地运行还是在服务器端运行都是一个确定的基准
            // 操作具体代码：
            // 1. 获取当前类的类加载器
            ClassLoader classLoader = JDBCUtils.class.getClassLoader();

            //2. 通过类加载器对象从类路径跟目录下读取文件
