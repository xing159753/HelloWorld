        ...
        /*
          ...
          3. 执行putVal
             public V put(K key, V value) {//K = "java" value = 10
                 retrun putVal(hash(key), key, value, false, true);
             }

             final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                        boolean evict) {
                Node<K,V>[] tab; Node<K,V> p; int n, i;//辅助变量
                //如果底层的table 数组是null, 或者 length = 0，就扩容到16
                if((tab = table) == null || (n = tab.length) == 0)
                    n = (table = resize()).length;
                    //取出hash值对应的table的索引位置的Node，如果为null把加入k-v
                    //, 创建成一个 Node, 加入该位置即可
                        ...
                        //如果找到的节点，后面是链表，就旬环比表
                            for (int binCount = 0; ; ++binCount) {//死循环
                                if(e = p.next) == null) {//如果整个链表，没有和他相同，就加到该链表的最后
                                    ...
                                    //判断当前链表的个数，是否已经到8个，到8个后，
                                    //就调用 treeifyBin 方法进行红黑树的转换
                                    if(binCount >= TREEIFY_THRESHOLD -1) // -1 for lst
                                        treeifyBin(tab, hash);
                                    break;
                                }
